...
class BlockUsual:
    def __init__(self, pos_x, pos_y, size, screen, image=None, additionally=None):
        ...
        if self.image is None:
            ...
            self.sprite.rect = pygame.Rect(*self.start_pos, *self.size)
    ...
    def draw(self):
        ...
        if self.image is None:
            ...
            self.sprite.rect = pygame.Rect(self.now_pos[0], self.now_pos[1], *self.size)
            self.shell = self.sprite.rect
...
class Main(BlockUsual):
    def __init__(self,pos_x, pos_y, size, screen, image=None, additionally=None):
        super().__init__(pos_x, pos_y, size, screen, '../textures/background/ground.png', additionally)
        self.size = [size, 150]
        self.sprite = pygame.sprite.Sprite()
        self.sprite.image = load_image(image)
        self.sprite.image = pygame.transform.scale(self.sprite.image, (self.size[0], self.size[1]))
