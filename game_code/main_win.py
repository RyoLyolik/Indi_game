...
class Window:
    def __init__(self, lvl, show_mountains=1):
        global screen
        self.show_mountains = show_mountains
        self.lvl = str(lvl)
        self.inv = Invent(8, 5)
    ...
    def key_events(self,e=None):
        if self.player.player.top - 120 < 0 and self.player.in_air:
            if self.level_data[0].shell.top - self.player.player.top > 150 and self.player.speed_down < 0:
                ...
                for i in self.mountains:
                    i.pos[1] -= self.player.speed_down
                for entity in self.level_data:
                    entity.now_pos[1] -= self.player.speed_down
        elif self.player.player.bottom + 120 > h and self.player.speed_down > 0:
            self.player.player.top = self.player.player.top - self.player.speed_down
            for i in self.mountains:
                i.pos[1] -= self.player.speed_down
            for entity in self.level_data:
        ...
        if self.player.player.right + 300 > w:
            for i in self.mountains:
                if i.layer == 1:
                    i.pos[0] -= self.player.speed*0.6
                elif i.layer == 2:
                    i.pos[0] -= self.player.speed * 0.3
                elif i.layer == 3:
                    i.pos[0] -= self.player.speed * 0.2
            for entity in self.level_data:
